...
namespace Graphics
{
    public class Model
    {
        ...
        public Model(List<vec3> vertices, List<vec2> uvCoordinates,
            List<vec3> normals)
        {
        ...
        public void Draw(int matID,mat4 scale,mat4 rot, mat4 trans)
        {
            ...
            if (texture != null)
                texture.Bind();
            if (indices.Count == 0)
                Gl.glDrawArrays(Gl.GL_QUADS, 0, vertices.Count);
            else
                Gl.glDrawElements(Gl.GL_QUADS, indices.Count, Gl.GL_UNSIGNED_SHORT, IntPtr.Zero);
